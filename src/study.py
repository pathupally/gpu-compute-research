...
def collect_contracts(paths: dict[str, list[tuple[str, float]]], kalshi: dict) -> list[dict]:
    ...
    for series_ticker, (gpu, actual_rule) in SERIES.items():
        ...
        market_by_ticker = {market["ticker"]: market for market in doc["markets"]}
...
def write_picture_chart(path: Path, labels: list[str], first: list[float], second: list[float],
    ...
    lines = ["% Generated by novelty/src/study.py; do not edit.",
...
def write_macros(audit: dict, bounds: list[dict], frontier: list[dict], core_frontier: list[dict]) -> None:
    ...
    core_mean_5 = next(row for row in core_frontier if row["rule"] == "month_mean"
                       and row["interpretation"] == "false_positive" and row["epsilon_pct"] == 5.0)
    pct = lambda value: f"{value:.1%}".replace("%", "\\%")
    lines = [
...
def main() -> None:
    ...
    payload = {
        "vintage": "2026-08-29", "generated_by": "novelty/src/study.py",
        "definitions": {
    ...
    write_macros(audit, bounds, frontier, core_frontier)
